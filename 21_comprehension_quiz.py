a = [[96], [69]]
print(''.join(list(map(str, a)))) #a joined string
...
def to_mod_list(employee_list):
   ...
   map_mod = map(mod, employee_list)
   mod_list = [ x for x in map_mod]
   return mod_list
...
def map_id_to_initial(employee_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   initial_list = [(key.get("name")[0]) for key in employee_list]
   id_list = [value.get('id') for value in employee_list]
   id_to_initial_dict = {key:value for (key, value) in zip(initial_list, id_list)}
   return id_to_initial_dict
   raise NotImplementedError()
